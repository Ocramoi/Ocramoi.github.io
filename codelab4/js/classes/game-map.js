...
let levelEl = document.getElementById("nivel");
...
class GameMap extends Entity {
	...
	*/
	constructor (containerElement) {
		...
		this.floor.rootElement.style.border = '1px solid green';
		this.floor.rootElement.style.zIndex = '-6';
	...
	*/
	initializeLevel () {
		timerInt = setInterval(() => { 
			...
			if(currentTime <= 0)
				this.gameOver();
			document.getElementById("timer").innerText = currentTime + "s";
		}, 1000)
		...
		if (Math.random() > 0.5)
			this.generateItem("faster")
		
		if (Math.random() > 0.65)
			this.generateItem("more");
	}
	...
	updateNivel(nivel)
	{
		levelEl.innerText = `Nível: ${nivel+1}`;
	}
	...
	*/
	generateItem (itemType) {
		...
		else if(itemType === 'faster' || itemType === 'more') element = new PowerUp(this.containerElement, Vector.zero, itemType);
