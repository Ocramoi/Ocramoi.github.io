...
class Hook extends MovableEntity {
	...
	*/
	pullBack () {
		...
		if (this.hookedObject) {
			...
			const speed = BASE_HOOK_PULL_SPEED * this.hookedObject.calculateHookSpeedMultiplier() * (this.velUp ? 10 : 1);
	...
	*/
	swing () {
		this.turn(0.0075 * this.swingDirection);
	...
	*/
	collided (object) {
		...
		else if(object instanceof PowerUp)
		{
			...
			object.delete();
			if(object.powerUpType === "faster")
			{
				this.velUp = true;
				setTimeout(() => this.velUp = false, 15000);
				this.pullBack();
			}
			else
			{
				let temp = 0 + soltarQnt;
				soltarQnt += 999;
				document.getElementById("soltar").innerText = "Soltar: ∞";
				setTimeout(() => {
					soltarQnt = temp;
					document.getElementById("soltar").innerText = "Soltar: " + temp;
				}, 15000);
				this.pullBack();
			}
		}
	...
	*/
	...
	explode()
	{
		let objPul = this.hookedObject;
		this.hookedObject = null;
		this.stopPulling();
		objPul.rootElement.style.backgroundImage = "url('assets/explosion.gif')";
		objPul.size = objPul.size.scale(2);
	...
	swim()
	{
		let clone = new Entity(movableEntityContainer, this.hookedObject.size, this.hookedObject.position, this.hookedObject.direction);
		clone.rootElement.style.backgroundImage = `url('assets/bass${this.hookedObject.imgDir}.png')`;
		this.stopPulling();
		let timeout = setInterval(() => {
			clone.position = new Vector(clone.position.x+(clone.position.x > 0 ? 1 : -1), clone.position.y);
			if(clone.position.magnitude() > 200)
			{
	...
	release()
	{
		if (this.hookedObject instanceof Gold) this.swim();
		else if(this.hookedObject instanceof Rock) this.explode();
		return false;
	}
}